...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedBookings = bookings?.map((booking: any) => ({
      _id: booking.id,
      id: booking.id,
      customerName: profilesMap[booking.user_id]?.full_name || 'Unknown',
      customerEmail: profilesMap[booking.user_id]?.email || '',
      customerPhone: profilesMap[booking.user_id]?.phone || '',
      carName: carsMap[booking.car_id]?.name || 'Unknown',
